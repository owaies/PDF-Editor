...
import { ChevronDown, ChevronLeft, ChevronRight, Download, FilePlus2, FileText, Highlighter, ImagePlus, MousePointer2, PenLine, Redo2, Search, Square, Trash2, Type, Undo2, ZoomIn, ZoomOut } from 'lucide-react';
import { create } from 'zustand';
import { inspectTextSpans, type TextSpan } from './services/pdfApi';
...
export default function App(){
  const [pdf,setPdf]=useState<PDFDocumentProxy|null>(null),[fileName,setFileName]=useState('Untitled.pdf'),[pages,setPages]=useState<PageState[]>([]),[currentPage,setCurrentPage]=useState(0),[zoom,setZoom]=useState(1),[status,setStatus]=useState('Open a PDF to begin'),[searchOpen,setSearchOpen]=useState(false),[query,setQuery]=useState(''),[dark,setDark]=useState(false),[textSpans,setTextSpans]=useState<TextSpan[]>([]);
  ...
  const openFile=useCallback(async(file:File)=>{
    ...
    try{
      ...
      setPdf(loaded);setPages(meta);setFileName(file.name);setCurrentPage(0);setZoom(1);setTextSpans([]);useEditor.getState().clearHistory();setStatus(`${loaded.numPages} page${loaded.numPages===1?'':'s'} loaded`);
      void inspectTextSpans(file).then(spans=>setTextSpans(spans)).catch(()=>setStatus(s=>s+' · text editing backend unavailable, browser overlay mode active'));
  ...
  const exportPdf=useCallback(async()=>{
    ...
    try{
      ...
      const doc=await PDFDocument.load(bytes),font=await doc.embedFont(StandardFonts.Helvetica);
      for(const o of objects){const page=doc.getPage(o.pageIndex),y=page.getHeight()-o.y-o.height;
        if(o.type==='text'){
          if(o.sourceSpanId&&o.originalText!==o.text){page.drawRectangle({x:o.x,y,width:o.width,height:o.height,color:rgb(1,1,1),borderWidth:0});}
      ...
      const out=await doc.save({useObjectStreams:true}),url=URL.createObjectURL(new Blob([out as BlobPart],{type:'application/pdf'})),a=document.createElement('a');a.href=url;a.download=fileName.replace(/\.pdf$/i,'')+'-edited.pdf';a.click();URL.revokeObjectURL(url);setStatus('Saved locally');
  ...
  const addImage=(file:File)=>{const reader=new FileReader();reader.onload=()=>add({id:makeId(),type:'image',pageIndex:currentPage,x:50,y:50,width:220,height:160,rotation:0,dataUrl:String(reader.result)});reader.readAsDataURL(file)};
  const addExistingText=(span:TextSpan)=>{if(objects.some(o=>o.type==='text'&&o.sourceSpanId===span.id)){select(objects.find(o=>o.type==='text'&&o.sourceSpanId===span.id)!.id);return}snapshot();add({id:makeId(),type:'text',pageIndex:span.pageIndex,x:span.bbox[0],y:span.bbox[1],width:Math.max(12,span.bbox[2]-span.bbox[0]),height:Math.max(12,span.bbox[3]-span.bbox[1]),rotation:0,text:span.text,fontSize:span.size||11,color:intColorToHex(span.color),sourceSpanId:span.id,originalText:span.text,sourceFont:span.font,sourceBBox:span.bbox})};
  ...
  return <div className={dark?'app dark':'app'}>
    ...
    <header className="topbar"><div className="brand"><div className="brandMark"><FileText size={18}/></div><span>PDF Editor</span></div><button className="menuBtn" onClick={()=>fileRef.current?.click()}><FilePlus2 size={17}/> Open PDF</button><div className="topActions"><IconButton label="Undo" onClick={undoAction}><Undo2/></IconButton><IconButton label="Redo" onClick={redoAction}><Redo2/></IconButton><div className="divider"/><button className="saveBtn" onClick={()=>void exportPdf()} disabled={!pdf}><Download size={16}/> Export PDF</button><button className="themeBtn" onClick={()=>setDark(v=>!v)}>{dark?'Light':'Dark'}</button></div></header>
