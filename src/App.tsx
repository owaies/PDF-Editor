...
import { getDocument, GlobalWorkerOptions, PDFDocumentProxy, type TextContentItem } from 'pdfjs-dist';
...
interface DrawObject extends BaseObject { type: 'draw'; points: Point[]; stroke: string; strokeWidth: number }
...
export default function App() {
  ...
  const openFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      setPdf(loaded); setPages(meta); setFileName(file.name); setCurrentPage(0); setZoom(1); setTextSpans([]); setBackendReady(false); useEditor.getState().clearHistory(); useEditor.getState().select(null);
      try { const spans = await inspectTextSpans(file); setTextSpans(spans); setBackendReady(true); setStatus(`${loaded.numPages} page${loaded.numPages === 1 ? '' : 's'} loaded · high-fidelity text editing ready`); }
      catch (e) { console.warn('Structural text backend unavailable, using PDF.js text extraction.', e); const spans = await extractLocalTextSpans(loaded, meta); setTextSpans(spans); setBackendReady(false); setStatus(`${loaded.numPages} page${loaded.numPages === 1 ? '' : 's'} loaded · text selection ready · local engine unavailable`); }
    } catch (e) { console.error(e); setStatus('Could not open this PDF. It may be malformed or encrypted.'); }
  }, []);

  const exportPdf = useCallback(async () => {
    if (!pdf) return; setStatus('Preparing PDF…');
    try {
      const bytes = (window as WindowPdf).__pdfBytes; if (!bytes) { setStatus('Please reopen the PDF before exporting.'); return; }
      const sourceTextEdits = objects.filter((o): o is TextObject => o.type === 'text' && !!o.sourceSpanId && !!o.sourceBBox && (o.originalText !== o.text || o.x !== o.sourceBBox[0] || o.y !== o.sourceBBox[1] || o.width !== (o.sourceBBox[2] - o.sourceBBox[0]) || o.height !== (o.sourceBBox[3] - o.sourceBBox[1]) || o.fontSize !== (o.sourceFontSize ?? o.fontSize) || o.color !== (o.sourceColor ?? o.color)));
      let workingBytes = bytes;
      if (sourceTextEdits.length) {
        if (!backendReady) { setStatus('Existing-text export needs the local PDF engine. New annotations can still be exported.'); return; }
      ...
      const doc = await PDFDocument.load(workingBytes), font = await doc.embedFont(StandardFonts.Helvetica);
      for (const o of objects) {
        const page = doc.getPage(o.pageIndex), y = page.getHeight() - o.y - o.height;
        if (o.type === 'text') { if (o.sourceSpanId) continue; page.drawText(o.text, { x: o.x, y, size: o.fontSize, font, color: hexToRgb(o.color), maxWidth: o.width }); }
        ...
        else if (o.type === 'image') { const img = o.dataUrl.startsWith('data:image/png') ? await doc.embedPng(o.dataUrl) : await doc.embedJpg(o.dataUrl); page.drawImage(img, { x: o.x, y, width: o.width, height: o.height, rotate: degrees(o.rotation) }); }
        else if (o.type === 'draw') for (let i = 1; i < o.points.length; i++) { const a = o.points[i - 1], b = o.points[i]; page.drawLine({ start: { x: a.x, y: page.getHeight() - a.y }, end: { x: b.x, y: page.getHeight() - b.y }, thickness: o.strokeWidth, color: hexToRgb(o.stroke) }); }
      }
      const out = await doc.save({ useObjectStreams: true }), url = URL.createObjectURL(new Blob([out as BlobPart], { type: 'application/pdf' })), a = document.createElement('a'); a.href = url; a.download = fileName.replace(/\.pdf$/i, '') + '-edited.pdf'; a.click(); setTimeout(() => URL.revokeObjectURL(url), 1000); setStatus(sourceTextEdits.length ? 'High-fidelity PDF exported' : 'PDF exported locally');
  ...
  useEffect(() => { if (!pdf) return; const obs = new IntersectionObserver((es) => { const v = es.filter((e) => e.isIntersecting).sort((a, b) => b.intersectionRatio - a.intersectionRatio)[0]; if (v) setCurrentPage(Number((v.target as HTMLElement).dataset.page)); }, { root: scrollerRef.current, threshold: [0.25, 0.6, 0.9] }); pageRefs.current.forEach((e) => e && obs.observe(e)); return () => obs.disconnect(); }, [pdf, pages.length, zoom]);
  ...
  const addExistingText = (span: TextSpan) => { const existing = objects.find((o) => o.type === 'text' && o.sourceSpanId === span.id); if (existing) { select(existing.id); return; } snapshot(); add({ id: makeId(), type: 'text', pageIndex: span.pageIndex, x: span.bbox[0], y: span.bbox[1], width: Math.max(12, span.bbox[2] - span.bbox[0]), height: Math.max(12, span.bbox[3] - span.bbox[1]), rotation: 0, text: span.text, fontSize: span.size || 11, color: intColorToHex(span.color), sourceSpanId: span.id, originalText: span.text, sourceFont: span.font, sourceFontSize: span.size || 11, sourceColor: intColorToHex(span.color), sourceBBox: span.bbox }); setStatus(backendReady ? 'Existing text selected for editing.' : 'Text selected. Start the local PDF engine before exporting this existing-text edit.'); };
  const zoomBy = (d: number) => setZoom((z) => Math.min(4, Math.max(0.25, Math.round((z + d) * 100) / 100)));
  ...
  return <div className={dark ? 'app dark' : 'app'}>
    ...
    <main className="workspace"><aside className="sidebar"><div className="sideTitle"><span>Pages</span><span className="count">{pages.length}</span></div><div className="thumbs">{pages.map((p) => <button key={p.index} className={p.index === currentPage ? 'thumb active' : 'thumb'} onClick={() => pageRefs.current[p.index]?.scrollIntoView({ behavior: 'smooth', block: 'center' })}><PageThumb pdf={pdf} index={p.index} /><span>{p.index + 1}</span></button>)}</div></aside>
      ...
      <aside className="properties"><div className="sideTitle">Properties</div>{selected ? <Properties object={selected} onChange={(p) => { snapshot(); update(selected.id, p); }} /> : <div className="emptyProps"><MousePointer2 size={20} /><p>Select an object or click existing PDF text.</p><small>{backendReady ? 'Existing text is editable through the local PDF engine.' : 'PDF text can be selected locally. Start the local engine for structural existing-text export.'}</small></div>}</aside>
    </main>
    <footer className="statusbar"><span>{status}</span><span>Page {pdf ? currentPage + 1 : 0} / {pages.length}</span><div className="zoom"><button onClick={() => zoomBy(-0.1)}><ZoomOut size={15} /></button><span>{Math.round(zoom * 100)}%</span><button onClick={() => zoomBy(0.1)}><ZoomIn size={15} /></button></div></footer>
...
async function extractLocalTextSpans(pdf: PDFDocumentProxy, pages: PageState[]): Promise<TextSpan[]> {
  const spans: TextSpan[] = [];
  for (const page of pages) {
    const p = await pdf.getPage(page.index + 1), content = await p.getTextContent();
    for (let i = 0; i < content.items.length; i++) {
      const item = content.items[i] as TextContentItem;
      if (!('str' in item) || !item.str.trim()) continue;
      const t = item.transform, size = Math.max(1, Math.hypot(t[0], t[1])), x = t[4], top = page.height - t[5] - size, width = Math.max(2, item.width), height = Math.max(8, size * 1.15);
      spans.push({ id: `local-${page.index}-${i}`, pageIndex: page.index, text: item.str, bbox: [x, top, x + width, top + height], font: '', size, color: 0x111111, flags: 0, origin: [x, top] });
    }
  }
  return spans;
}
...
function PDFPage({ pdf, page, zoom, pageRef, tool, objects, textSpans, selectedId, onSelect, onExistingText, onAdd, onUpdate }: { pdf: PDFDocumentProxy; page: PageState; zoom: number; pageRef: (el: HTMLDivElement | null) => void; tool: Tool; objects: EditorObject[]; textSpans: TextSpan[]; selectedId: string | null; onSelect: (id: string | null) => void; onExistingText: (s: TextSpan) => void; onAdd: (o: EditorObject) => void; onUpdate: (id: string, p: Partial<EditorObject>) => void }) {
  const canvasRef = useRef<HTMLCanvasElement>(null), drawing = useRef(false), draftPoints = useRef<Point[]>([]);
  const [rendered, setRendered] = useState(false), [livePoints, setLivePoints] = useState<Point[]>([]);
  useEffect(() => { let cancelled = false; setRendered(false); (async () => { const p = await pdf.getPage(page.index + 1), viewport = p.getViewport({ scale: zoom * window.devicePixelRatio }), canvas = canvasRef.current; if (!canvas || cancelled) return; canvas.width = viewport.width; canvas.height = viewport.height; canvas.style.width = `${viewport.width / window.devicePixelRatio}px`; canvas.style.height = `${viewport.height / window.devicePixelRatio}px`; await p.render({ canvasContext: canvas.getContext('2d')!, viewport }).promise; if (!cancelled) setRendered(true); })(); return () => { cancelled = true; }; }, [pdf, page.index, zoom]);
  const cssW = page.width * zoom, cssH = page.height * zoom;
  const pointFromEvent = (e: PointerEvent<HTMLDivElement>): Point => { const r = e.currentTarget.getBoundingClientRect(); return { x: clamp((e.clientX - r.left) / zoom, 0, page.width), y: clamp((e.clientY - r.top) / zoom, 0, page.height) }; };
  const click = (e: MouseEvent<HTMLDivElement>) => { if (tool === 'select' || tool === 'draw') return; const r = e.currentTarget.getBoundingClientRect(), x = clamp((e.clientX - r.left) / zoom, 0, page.width), y = clamp((e.clientY - r.top) / zoom, 0, page.height); if (tool === 'text') onAdd({ id: makeId(), type: 'text', pageIndex: page.index, x: clamp(x, 0, Math.max(0, page.width - 180)), y: clamp(y, 0, Math.max(0, page.height - 24)), width: Math.min(180, page.width), height: 24, rotation: 0, text: 'Type here', fontSize: 14, color: '#111111' }); else if (tool === 'highlight') onAdd({ id: makeId(), type: 'highlight', pageIndex: page.index, x: clamp(x, 0, Math.max(0, page.width - 160)), y: clamp(y, 0, Math.max(0, page.height - 18)), width: Math.min(160, page.width), height: 18, rotation: 0, color: '#facc15', opacity: .35 }); else if (tool === 'shape') onAdd({ id: makeId(), type: 'shape', pageIndex: page.index, x: clamp(x, 0, Math.max(0, page.width - 160)), y: clamp(y, 0, Math.max(0, page.height - 90)), width: Math.min(160, page.width), height: Math.min(90, page.height), rotation: 0, stroke: '#111827', fill: '#ffffff00', strokeWidth: 1.5 }); };
  const startDraw = (e: PointerEvent<HTMLDivElement>) => { if (tool !== 'draw') return; e.preventDefault(); e.stopPropagation(); e.currentTarget.setPointerCapture(e.pointerId); drawing.current = true; const p = pointFromEvent(e); draftPoints.current = [p]; setLivePoints([p]); };
  const moveDraw = (e: PointerEvent<HTMLDivElement>) => { if (!drawing.current || tool !== 'draw') return; const p = pointFromEvent(e), last = draftPoints.current.at(-1); if (!last || Math.hypot(p.x - last.x, p.y - last.y) >= 1.5) { draftPoints.current.push(p); setLivePoints([...draftPoints.current]); } };
  const finishDraw = (e: PointerEvent<HTMLDivElement>) => { if (!drawing.current) return; e.preventDefault(); drawing.current = false; try { e.currentTarget.releasePointerCapture(e.pointerId); } catch {} const points = draftPoints.current; draftPoints.current = []; setLivePoints([]); if (points.length >= 2) { const xs = points.map((p) => p.x), ys = points.map((p) => p.y), minX = Math.min(...xs), minY = Math.min(...ys), maxX = Math.max(...xs), maxY = Math.max(...ys); onAdd({ id: makeId(), type: 'draw', pageIndex: page.index, x: minX, y: minY, width: Math.max(1, maxX - minX), height: Math.max(1, maxY - minY), rotation: 0, points, stroke: '#111827', strokeWidth: 2 }); } };
  const visibleSpans = textSpans.filter((s) => !objects.some((o) => o.type === 'text' && o.sourceSpanId === s.id));
  return <div className="pageShell" ref={pageRef} data-page={page.index}><div className="pdfPage" style={{ width: cssW, height: cssH }} onClick={click} onPointerDown={startDraw} onPointerMove={moveDraw} onPointerUp={finishDraw} onPointerCancel={finishDraw}>
    <canvas ref={canvasRef} />{!rendered && <div className="pageLoading">Rendering…</div>}
    {tool === 'draw' && livePoints.length > 1 && <svg className="drawPreview" viewBox={`0 0 ${page.width} ${page.height}`} preserveAspectRatio="none"><polyline points={livePoints.map((p) => `${p.x},${p.y}`).join(' ')} fill="none" stroke="#111827" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>}
    <div className="textSpanLayer">{tool === 'select' && visibleSpans.map((s) => <button key={s.id} className="textSpan" style={{ left: s.bbox[0] * zoom, top: s.bbox[1] * zoom, width: Math.max(4, (s.bbox[2] - s.bbox[0]) * zoom), height: Math.max(8, (s.bbox[3] - s.bbox[1]) * zoom) }} title={`Edit “${s.text}”`} onClick={(e) => { e.stopPropagation(); onExistingText(s); }}>{s.text}</button>)}</div>
...
function EditorObjectView({ object: o, zoom, selected, onSelect, onUpdate }: { object: EditorObject; zoom: number; selected: boolean; onSelect: (id: string) => void; onUpdate: (id: string, p: Partial<EditorObject>) => void }) {
  ...
  const beginDrag = (e: MouseEvent<HTMLDivElement>) => { const target = e.target as HTMLElement; if (target.closest('textarea') || target.closest('input') || target.closest('.resizeHandle')) return; e.stopPropagation(); onSelect(o.id); setDrag({ sx: e.clientX, sy: e.clientY, ox: o.x, oy: o.y }); };
  const common = { className: selected ? 'editorObject selected' : 'editorObject', style, onMouseDown: beginDrag };
  if (o.type === 'text') return <div {...common}><textarea autoFocus={selected} value={o.text} onChange={(e) => onUpdate(o.id, { text: e.target.value })} style={{ fontSize: o.fontSize * zoom, color: o.color, fontFamily: fontFamilyFor(o.sourceFont) }} /><div className="objectMoveHandle" title="Drag to move" /><ResizeHandle o={o} zoom={zoom} onUpdate={onUpdate} /></div>;
  ...
  if (o.type === 'draw') return <svg {...common} viewBox={`0 0 ${Math.max(1, o.width)} ${Math.max(1, o.height)}`} preserveAspectRatio="none"><polyline points={o.points.map((p) => `${p.x - o.x},${p.y - o.y}`).join(' ')} fill="none" stroke={o.stroke} strokeWidth={o.strokeWidth} strokeLinecap="round" strokeLinejoin="round" vectorEffect="non-scaling-stroke" /></svg>;
  return <div {...common}><img src={o.dataUrl} alt="Inserted" draggable={false} /><ResizeHandle o={o} zoom={zoom} onUpdate={onUpdate} /></div>;
}

function ResizeHandle({ o, zoom, onUpdate }: { o: EditorObject; zoom: number; onUpdate: (id: string, p: Partial<EditorObject>) => void }) { const start = useRef<{ x: number; y: number; w: number; h: number } | null>(null); return <span className="resizeHandle" onMouseDown={(e) => { e.stopPropagation(); start.current = { x: e.clientX, y: e.clientY, w: o.width, h: o.height }; const move = (ev: globalThis.MouseEvent) => { if (!start.current) return; onUpdate(o.id, { width: Math.max(20, start.current.w + (ev.clientX - start.current.x) / zoom), height: Math.max(10, start.current.h + (ev.clientY - start.current.y) / zoom) }); }; const up = () => { start.current = null; window.removeEventListener('mousemove', move); window.removeEventListener('mouseup', up); }; window.addEventListener('mousemove', move); window.addEventListener('mouseup', up); }} />; }
...
function ToolButton({ active, label, onClick, children }: { active: boolean; label: string; onClick: () => void; children: ReactNode }) { return <button className={active ? 'tool active' : 'tool'} onClick={onClick} title={label}>{children}<span>{label}</span></button>; }
function IconButton({ label, onClick, children }: { label: string; onClick: () => void; children: ReactNode }) { return <button className="iconBtn" title={label} onClick={onClick}>{children}</button>; }
...
function hexToInt(hex: string) { const h = hex.replace('#', ''), normalized = h.length === 3 ? h.split('').map((x) => x + x).join('') : h; return parseInt(normalized, 16) >>> 0; }
